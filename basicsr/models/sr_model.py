...
class SRModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        # self.output, kl_loss = self.net_g(self.lq)
    ...
    def test(self, dataset_name, img_name):
        ...
        if hasattr(self, 'net_g_ema'):
            ...
            with torch.no_grad():
                for i in range(100):
                    if i == 0:
                        y = np.array([self.net_g(self.lq).data.cpu().numpy()]).squeeze(0)
                        Yt_hat = y
                    else:
                        y = np.array([self.net_g(self.lq).data.cpu().numpy()]).squeeze(0)
                ...
                self.output = Yt_hat.mean(0)
                self.output = np.expand_dims(self.output, axis=0)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            # self.test(dataset_name, img_name)
            self.test()
